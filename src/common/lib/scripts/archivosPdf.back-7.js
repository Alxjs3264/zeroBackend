...
(async function (){
    ...
    for (let index = 0; index < result.length; index++) {
        ...
        const usuariosDerivaciones = res.map(d => ({idUsuario: d.id_usuario_destinatario, tipo: d.tipo}))
        ...
        if (derivaciones.length) {
            ...
            if (['DE','VIA'].includes(doc.ultimo_participante_nivel) && usuariosDerivaciones.length > doc.cantidad_usuarios_flujo) {
                let ultimoPatipanteFlujo;
                let conuterTemporal = 0
                for (let i = 0; i < usuarios.length; i++) {
                    const find = usuariosDerivaciones.find(it => it.idUsuario === usuarios[i])
                    if (!find) {
                        break
                    }
                    ultimoPatipanteFlujo = find
                    conuterTemporal ++;
                }
                if (ultimoPatipanteFlujo && usuarioActual.idUsuario !== ultimoPatipanteFlujo.idUsuario) {
                    doc.cantidad_usuarios_flujo = conuterTemporal
                    doc.flujo_completo = parseInt(usuarios.length) === conuterTemporal
                    doc.firmas_faltantes_flujo = parseInt(doc.cantidad_usuarios_flujo) - parseInt(doc.cantidad_firmas)
                    doc.estado_flujo = doc.flujo_completo ? 'COMPLETO' : 'INCOMPLETO'
                    // sadañlksjd asdp
                    usuarioActual = doc.configuracion_derivaciones.find(reg => reg.idUsuario === ultimoPatipanteFlujo.idUsuario)
                    doc.ultimo_participante = usuarioActual.nombreUsuario
                    const componente = doc.plantilla_valor.find(it => it.type === 'derivacion')
                    let ultimo_participante_nivel = ''
                    if (componente) {
                    ...
                    doc.ultimo_participante_nivel = ultimo_participante_nivel + ' - ' + ultimoPatipanteFlujo.tipo
                }
            }
