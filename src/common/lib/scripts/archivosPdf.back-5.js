...
(async function (){
    ...
    for (let index = 0; index < result.length; index++) {
        ...
        let subquery = `
        SELECT DISTINCT id_usuario_destinatario, fecha_recepcion, fecha_derivacion, _created_at
        FROM gestion_flujo_derivacion
        WHERE id_documento = '${doc.id}'
        AND _deleted_at IS NULL
        -- AND observacion = false
        AND tipo IN ('REMITENTE', 'VIA', 'DESTINATARIO', 'RECUPERADO')
        AND id_usuario_destinatario IN ('${usuarios.join('\',\'')}')
        ORDER BY _created_at DESC
        `
        let res = await getQueryResult(subquery);
        doc.cantidad_firmas = 0
        doc.cantidad_usuarios_flujo = res.length > usuarios.length ? usuarios.length : res.length
